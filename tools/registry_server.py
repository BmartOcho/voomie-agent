"""
registry_server.py — FastMCP server exposing two tools:
    query_stock_registry  → wraps shoptalk's tools/query-registry.rkt (stock)
    query_press_registry  → wraps shoptalk's tools/query-registry.rkt (press)
...
This server flattens that into a per-call response shape that mirrors
parse_shoptalk_server.py's success/failure discriminator pattern:
