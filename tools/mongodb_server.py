"""
mongodb_server.py — FastMCP server exposing six tools that share a single
MongoDB client: lookup_customer, create_customer, update_job_status,
append_conversation_turn, persist_job, flag_for_human.

All six tools share a module-level MongoClient initialized at import time
with the same retry/ping pattern as prototype-v0/run_agent.py's
get_collection() helper (serverSelectionTimeoutMS=5000, ping check,
3 retries with 2 s backoff). The ping forces a real round-trip so a
misconfigured URI fails fast at startup rather than on the first request.

If MONGODB_URI is unset or the cluster is unreachable, the server still
starts so MCP can advertise the tool surface; each tool then returns
{ok: false, error: "mongodb_unavailable"} until connectivity is restored.
...
stderr (visible in the parent terminal) so the operator can confirm
subprocess identity and connection health without polluting the
JSON-RPC channel.
"""
...
_client: MongoClient | None = _connect()
_db = _client[DB_NAME] if _client is not None else None


def _customers():
    return _db["customers"] if _db is not None else None


def _jobs():
    return _db["jobs"] if _db is not None else None


def _conversations():
    return _db["conversations"] if _db is not None else None


def _flags():
    return _db["flags"] if _db is not None else None
...
print(
    ...
    f"connected={_client is not None}",
    file=sys.stderr,
    flush=True,
)
